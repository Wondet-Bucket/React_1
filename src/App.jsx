...
function Refresh_products(props)
{
  ...
  return (
     <div className='prod_map'>
      {products_list.map((element) => (
        ...
        <h3>{element['model']}</h3>
        <p>${element['price']}</p>
        {(props.card_amount[element.id] || 0) === 0 ? 
        (
          <button className='add_to_card' onClick={() => {props.func(1); props.add(element.id, 1)}}>Add to card</button>
        )
        :
        (
          <div className='plus_minus'>
            <button className='add_to_card' onClick={() => {props.add(element.id, 1); props.func(1)}}>+</button>
            <p>{props.card_amount[element.id]}</p>
            <button className='add_to_card' onClick={() => {props.func(-1); props.add(element.id, -1)}}>-</button>
          </div>
        )}
        </div>
      ))}
...
function App() {
  const [prod_amount, setProduct] = useState(0);
  const addtoCard = (delta) => {
    setProduct(prev => prev + delta);
  };
  const [card_amount, setCardAmount] = useState({});
  const add_tovar = (productId, delta) => {
    setCardAmount(prev => ({
      ...prev,
  ...
  return (
    ...
    <header>
      <div className='header'>
        ...
        <div className='head_btns'>
          <p>Total amount of products - {prod_amount}</p>
    ...
    <div className="content">
      <div className="hor_container">
        ...
        <div className='products_zone'>
            <p>8 products</p>
            <div>
              <Refresh_products func={addtoCard} add={add_tovar} card_amount={card_amount}/>
            </div>
          <div >
          </div>
        </div>
